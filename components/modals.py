...
class GetWinnersCount(discord.ui.Modal):
    def __init__(self, *args, **kwargs):
        self.message = kwargs.pop(
            "message"
        )  # or could do __init__(self, message, *args, **kwargs)
        super().__init__(*args, **kwargs)
        self.add_item(
            discord.ui.InputText(
                label="Number of winners: ", placeholder="Number of winners"
            )
        )
    ...
    async def callback(self, interaction):
        ...
        with interaction.channel.typing():
            ...
            participants_formatted = ", ".join(
                [str(i) for i in participants]
            ...
            participants_formatted = participants_formatted[
                : participants_formatted.rfind(",")
            ]  # don't combine the above line as .find will get wrong index. first shorten and assign  then find
            content = f":tada:<:juuzou_gaming:1125994304528187392>**Giveaway Winner Announcement!**<:juuzou_gaming:1125994304528187392>:tada:\nThe winners are:\n**<@!{'> ,<ENTERCHR101><@!'.join([str(ab) for ab in random.sample(participants, winners)])}>**\n\n:partying_face:Congratulations!!:partying_face:\n\nParticipants: ```{participants_formatted} {a}```".replace(
                "<ENTERCHR101>", "\n"
            )
        view = ConfirmWinners()
